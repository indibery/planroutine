  ...
  group('저장할 캘린더 고르기', () {
    test('실기기 구성 — 로컬이 먼저 와도 구글 계정 캘린더를 고른다', () {
      ...
      expect(
        ...
        '11',
        reason:
            '로컬 My calendar(id=1)가 목록 앞에 있고 isDefault도 true지만, '
            '계정 캘린더가 있으면 그쪽이 정답이다',
      );
    });

    test('계정 캘린더가 없으면 로컬을 쓴다 — 금지가 아니라 후순위다', () {
      final localOnly = _galaxyA34
          .where((c) => c.accountType == 'LOCAL')
          .toList();
      expect(
        pickDefaultCalendar(localOnly)?.id,
        '1',
    ...
    test('읽기 전용은 후보에서 빠진다', () {
      ...
      expect(
        pickDefaultCalendar(readOnlyGoogle)?.id,
        '1',
