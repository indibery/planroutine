...
void main() {
  group('items의 세 형태', () {
    test('List — 복수 응답', () {
      final r = parseArrivals(
        _envelope([_arr(92, 'A', 600), _arr('92-1', 'B', 120)]),
      );
  ...
  group('노선 축약 — routeid별 arrtime 최소만 남긴다', () {
    test('같은 노선 2건이면 빠른 쪽만 남는다', () {
      final r = parseArrivals(
        _envelope([_arr(92, 'A', 1500), _arr(92, 'A', 600)]),
      );
  ...
  group('도착 시각 변환', () {
    test('초를 그대로 보존한다 — 시간 축이 이 정밀도로 점을 놓는다', () {
      ...
      expect(
        parseArrivals(_envelope(_arr(1, 'A', 551))).items.single.arrSec,
        551,
      );
      expect(
        parseArrivals(_envelope(_arr(1, 'A', 61))).items.single.arrSec,
        61,
      );
    });
    ...
    test('분은 반올림으로 파생된다', () {
      ...
      expect(
        parseArrivals(_envelope(_arr(1, 'A', 551))).items.single.arrMin,
        9,
      );
