...
| 버스 정류장 설정 | 정류장 ID·이름·번호·도시코드·선택 노선, 출퇴근 시간대 | 사용자가 정류장을 등록할 때 | 기기 내 `shared_preferences` |
...
- **네트워크 통신**: Google Calendar API 통신과 국토교통부 TAGO 조회는 전부
  HTTPS(TLS 1.2 이상, TAGO는 TLS 1.3 실측)로 이루어집니다. 이 둘이 본 앱의
  외부 통신 전부입니다.
