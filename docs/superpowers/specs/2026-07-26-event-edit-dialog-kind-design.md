...
```
[accent bar] ┌ [업무] ★ 제목                         [연도 배지] [✓]
             └ 설명
```

### `★ 중요` 배지 줄 제거 → ★ 아이콘 인라인

중요 표시는 이미 **골드 신호 네 겹**이다: 레일(`:174`) · 카드 배경 14%(`:187`) ·
테두리 50%(`:192`) · `★ 중요` 배지(`:213`). 앞의 셋은 세로를 안 쓰는데 배지만 약 25px을
쓴다(중요 행이 43~66px → 68~91px, 약 35% 증가). 골드가 셋이나 말하고 있는데 pill 안에
`중요`라는 **글자까지** 읽힐 이유는 없다.

- `_buildImportantBadge`(제목 위 `_goldPill`)를 없애고, **★ 아이콘만** 종류 배지 뒤·제목 앞에
  인라인으로 둔다. `Icons.star_rounded`, size 14, `AppColors.gold`.
- **★ 자체는 지우지 않는다.** `:171` 주석과 2026-07-03 스펙이 "중요는 색이 아닌 형태(★)로
  구분"을 명시적 결정으로 남겼다. ★를 빼면 남는 신호가 전부 색이라 색각 이상 사용자에게는
  구분이 사라진다.
- 글자가 사라지므로 `Semantics(label: CalendarStrings.importantBadge)`로 감싼다.
  문자열 상수는 그대로 둔다(스크린리더 라벨로 계속 쓰인다).
- `_goldPill`은 연도 배지가 계속 쓰므로 남긴다.
- 결과: **중요 행도 보통 행과 같은 높이**가 된다.
...
- 중요 이벤트 행: `event_important_badge_<id>` 키가 ★ 아이콘 위젯으로 남아 있고,
  `중요` **텍스트는 더 이상 없다**
- 중요 + 종류 배지 + 연도 배지가 한 행에 다 있어도 제목이 넘치지 않는다(overflow 없음)
- 중요 행과 보통 행의 높이가 같다

**기존 E2E 수정 필요**: `integration_test/app_test.dart:350`이
`find.text(CalendarStrings.importantBadge)`로 배지를 찾는다. 글자가 사라지므로
`find.byKey(Key('event_important_badge_<id>'))`로 바꾼다.
...
- **중요 배지 → ★ 인라인**: 같은 사실을 네 번 말하던 것 중 유일하게 세로를 쓰던 하나를 줄인다.
  중복 신호를 지울 때 **색이 아니라 글자 쪽을 지우는** 이유는, 남은 셋이 전부 색이라
  형태 신호(★)가 마지막 비색상 단서이기 때문이다.
