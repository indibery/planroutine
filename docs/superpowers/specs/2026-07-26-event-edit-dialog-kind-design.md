...
- **캘린더 목록의 종류 배지** — `month_event_list`·`event_list_section` 어디에도 `kind` 표시가
  없어 캘린더에서 업무/학교일정을 구분할 수 없다. "왜 오늘 탭에 떴지?"의 나머지 절반이지만,
  이번 요청(추가 시트)의 범위 밖이라 뺀다. 별도 작업으로 판단.
- 종류별 격자 점 색 구분 — 색 규칙이 이미 포화(골드=오늘·중요, 빨강=공휴일·일요일,
  파랑=토요일·이벤트).
