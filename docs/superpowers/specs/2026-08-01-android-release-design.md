...
성립하지 않았다** — 9분 규칙을 무해하게 만드는 것은 이 병합 구조다.

⚠️ **9분 규칙에 안 걸리는 것이 정시 발화를 뜻하지는 않는다.** 그것은 *발화 빈도* 상한이고,
`setAndAllowWhileIdle`은 그와 별개로 **inexact alarm**이다 — 시스템이 알람을 묶어서 발화하고
공식 문서에 **지연 상한이 없다.** 그래서 브리프의 "08:00~08:15"는 근거보다 강한 표현이었다
(9분 규칙을 지연 폭으로 잘못 읽은 것이다). 이 스펙이 약속할 수 있는 것은
**"정시가 아니다. 지연 폭은 확정할 수 없다"** 까지다.

**iOS와의 차이는 여기 하나뿐이다.** iOS `zonedSchedule`은 `UNCalendarNotificationTrigger`로
등록되어(`FlutterLocalNotificationsPlugin.m:538-548`) 시스템이 정시 발화를 보장한다.
`androidScheduleMode`는 **iOS 경로가 읽지 않는 파라미터**라(같은 파일에 참조 0건)
현재 코드의 `exactAllowWhileIdle`은 지금까지 한 번도 쓰인 적이 없다 — iOS 전용 배포였으므로.
inexact로 바꿔도 **iOS 알림 동작은 1초도 바뀌지 않는다.**
...
- **제조사 배터리 최적화** — 가장 큰 실전 리스크. 갤럭시 사용자에게 "며칠 안 열고 뒀다가 아침
  ...
  maintenance window로 연기"까지이고, **inexact alarm의 실제 지연 폭이 얼마인지는
  에뮬레이터로도 확인할 수 없다.**
