...
class BusStrings {
  ...
  /// "· B정류장" — 제목줄에서 방향 이름 뒤에 붙는 정류장 세그먼트.
  ...
  ///
  /// **예시는 서울의 공공 랜드마크로 둔다.** 예전 예시(`26044`)는 개발자 집 근처
  /// 정류장이었다 — 앱에 박힌 예시는 사용자 수만큼 읽히므로 개인 위치를 흘리면 안 된다.
  ///
  /// 서울 번호는 **구조적으로 1곳이 되지 않는다**: 서울·경기가 5자리 번호 공간을
  /// 공유해 같은 번호의 경기 정류장이 늘 함께 나온다(실측 `14016`·`22173`·`01009`
  /// 모두 2곳). `26044`가 1곳이었던 것은 서울 번호 대역 밖이라 겹칠 짝이 없어서였다.
  /// 그래서 [searchCapitalHint]도 `한 곳`이 아니라 `한두 곳`이라고 말한다.
  static const stopSearchHint = '정류소번호 또는 이름 (예: 02004)';
