...
///
/// ⚠️ **지난 해를 지우면 이름만 사라지는 것이 아니다.** `calendar_day_cell.dart`가
/// [isKoreanHoliday]로 **빨간색**을 정하므로 그 해 추석은 검은 평일로 그려진다.
/// `CalendarMonthPager`의 `PageView`에 `itemCount`가 없어 **월 이동에 하한이 없다** —
/// 뒤로 넘기면 그 화면에 도달한다.
///
/// ⚠️ **가드가 막는 것은 "크기 때문에 솎는 것"이 아니다.** 연도당 10줄이라 표가 커져서
/// 정리해야 할 상황은 오지 않는다. 실제 위험은 **표를 다시 쓸 때**다 — 2028년을 넣으려고
/// 블록을 갈아쓰면서 2025년을 조용히 빠뜨리는 것. 지워서 얻는 것이 0이고 잃는 것은 위
/// 화면이므로, 남기는 이유는 "필요해서"가 아니라 **"버리는 값이 없어서"** 다.
/// (2027년에 설치한 사용자가 2025년을 볼 일은 드물다 — 그 점은 사실이다.)
///
