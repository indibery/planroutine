...
/// 기한 아닌 문장을 일정으로 만든다.
///
/// **제목 규칙도 업무 프롬프트 안에서 다시 갈린다**(2026-08-14). `그대로 쓰라`는
/// (가) 손글씨 목록에만 건다 — 세 줄짜리 메모가 한 줄로 뭉갠 사고(2026-08-07)를 막는
/// 규칙이라 거기서는 필수다. 반대로 (나) 공문·안내문은 본문이 **경어체 문장**이라
/// 그대로 옮기면 `설문에 참여하여 주시기 바랍니다`가 제목이 된다(실측). 그래서 (나)에는
/// **할 일 이름으로 짧게** 쓰되 사진에 없는 말은 지어내지 말라고 따로 지시한다.
/// 한 규칙을 공통에 두면 두 맥락 중 한쪽이 반드시 틀린다.
///
...
- title은 적힌 문구를 **그대로** 씁니다. 줄이거나 바꿔 쓰지 않습니다.
...
- title은 **할 일 이름**으로 짧게 씁니다. 문서 문장을 그대로 옮기지 않습니다 — `~해 주시기 바랍니다`, `~하시기 바랍니다` 같은 서술은 이름이 아닙니다.
  사진에 있는 낱말만 써서 만들고, 사진에 없는 말을 **지어내지 않습니다**.
  문서에 이름이 적혀 있으면(`행사명:`, `제목:` 등) 그 이름을 씁니다.
  예: `설문에 참여하여 주시기 바랍니다` → `발대식 참석 설문 참여`
...
- **날짜를 가리키는 말은 title에서 뺍니다.** `(오늘)`, `(8월 9일까지)` 같은 표현은 date로 옮겨갔으므로 제목에 남기지 않습니다.
