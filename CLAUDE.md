...
4. **검토 후 확정** — 입력 탭은 **검토 대기만** 보여준다(필터 없음). 슬라이드로 확정(→) / 삭제(←). 하단 `일괄 업무 확정 N건` / `일괄 행사 확정 N건` pill로 종류별 일괄 확정(해당 종류 0건이면 숨김). 확정 시 캘린더 이벤트 자동 생성(종류 승계)되고 이 목록에서 빠진다 — **행은 남지만 캘린더와 운명을 같이한다**(내보내기·`작년` 배지·중복 체크가 보고, 캘린더에서 그 이벤트를 지우면 원본도 함께 내려간다). 대기가 없으면 문구 한 줄.
...
6. **휴지통** — 일정/이벤트 soft-delete, 30일 후 자동 영구 삭제. 캘린더에서 지운 항목은 **이벤트 한 줄로만** 뜬다(짝인 원본 일정은 감춘다).
...
- **캘린더 이벤트의 삭제·복구·영구삭제는 연결된 원본 일정까지 연쇄한다**(아래 절).
...
- 대기 0이면 **빈 상태 두 줄**(`검토할 일정이 없습니다` / `위에서 사진이나 작년 CSV로
  넣어보세요`).
  - ⚠️ `등록된 일정이 없습니다`로 되돌리면 안 된다 — 목록이 대기 전용이 된 뒤로는 확정
    72건이 있어도 이 목록이 빌 수 있어 **거짓**이 된다(2026-09-03). 가드가 막는다.
  - **힌트 줄이 필수다.** 대기 0이면 하단 확정 pill도 스와이프 안내 바도 안 떠서
    이 두 줄이 **화면의 전부**다. 다른 빈 상태(오늘 탭·버스 카드)도 모두 상태 + 다음
    행동 두 줄이고, 입력 탭만 한 줄이었다(2026-09-04 실기기 신고로 추가).
  - ⚠️ **`검토`는 빈 화면에서 처음 만나는 낱말이다.** 필터를 걷어내면서 `검토 대기 N`
    칩이 사라져 화면 위에 그것을 설명하는 것이 없어졌고, 남은 자리는 확정·삭제
    다이얼로그와 가져오기 스낵바뿐인데 셋 다 목록에 항목이 있어야 보인다. **알고
    유지한다**(사용자 결정 2026-09-04) — 힌트 줄이 다음 행동을 알려주므로 넣어보면
    목록이 차고, 그 순간 pill과 안내 바가 나타나 낱말이 화면에서 설명된다. 빈 화면에서
    알아야 하는 것은 `검토`의 뜻이 아니라 **무엇을 하면 되는지**다.
  - **일반화**: 기능을 걷어낼 때 **그 기능이 조용히 가르치던 낱말도 함께 사라진다.**
    남은 문구가 그 낱말에 기대고 있는지 확인할 것 — 여기서는 확인 끝에 남기기로 했지만,
    확인 자체를 안 하면 선택지가 있다는 것도 모른다.
  - 가드는 `schedule_screen_review_test.dart` 2건 — 힌트 줄 존재 + `empty`가 `등록된`을
    담지 않는지. **`검토`는 금지하지 않는다**(위 결정).
...
#### 확정 행은 쌓인다 — 지우는 길은 **캘린더 하나**다

필터를 없앤 뒤의 구조적 귀결이다. 사용자 확인 후 **쌓이는 것 자체는 그대로 두기로 했고**
(2026-09-03), 뒤에 캘린더 삭제가 원본까지 연쇄하게 되면서 지울 길이 하나 생겼다(2026-09-11).
...
- **개별 확정 행을 지우는 길은 캘린더다**(2026-09-11). `ScheduleRepository.deleteSchedule(id)`의
  호출부는 여전히 입력 탭의 ← 스와이프 하나뿐이지만, 그 목록은 대기 전용이다. 대신
  `CalendarRepository.deleteEvent`가 연결된 원본까지 내려보내므로 **확정된 일정은 캘린더에서
  지운다.** 한동안은 그 길도 없어서(`calendar_events`만 지우고 `schedules`는 남겼다) 지운
  행사를 다시 넣을 수 없었다 — 아래 절이 그 결함과 수정을 적는다.
...
#### 캘린더 삭제는 원본 일정까지 연쇄한다 (2026-09-11)

실기기 신고: **"캘린더에서 지운 행사를 다시 넣으면 중복이라며 안 들어간다."**

원인은 두 테이블이 따로 움직인 것이다. `deleteEvent`가 `calendar_events`만 soft-delete하고
`schedules` 원본을 활성으로 남겼는데, 중복 판정은 그 원본을 본다
(`insertConfirmedOrPending`·`createBulkFromImported`의 `title + scheduled_date`,
`ai_photo_flow`의 사전 필터가 모두 `deleted_at IS NULL` 기준). 그래서 **사용자 화면에는
캘린더가 비어 있는데 앱은 "이미 있다"며 조용히 스킵**했다 — 등록 버튼을 눌러도 목록이
그대로라 눌린 건지조차 알기 어려웠다.

**고친 방향은 "중복 판정이 캘린더를 보게 한다"가 아니라 "지우면 같이 지운다"이다**
(사용자 결정). `schedules` 행은 작년 일정을 참조하기 위한 내부 기록일 뿐이고, 사용자에게
보이는 캘린더가 기준이어야 한다는 것이다. 그래서 화면에 나타나는 두 곳(휴지통·CSV
내보내기)도 캘린더를 따른다.

- **연쇄는 `CalendarRepository`의 세 메서드 전부에 건다** — `deleteEvent`·`restoreEvent`·
  `permanentDeleteEvent`. 삭제만 걸면 복구했을 때 이벤트만 돌아오고 원본은 휴지통에 남아
  **중복 판정이 되살아난다.** 셋 다 `db.transaction`으로 묶어 한쪽만 반영되는 상태를 없앤다.
  - 짝 찾기는 `_linkedScheduleId`(= `calendar_events.schedule_id`) 한 곳이다. 손입력 이벤트는
    이 값이 null이라 아무 일정도 건드리지 않는다.
- **CSV 내보내기는 고치지 않았다.** 쿼리가 이미 `deleted_at IS NULL AND status='confirmed'`라
  원본이 함께 내려가면 자동으로 빠진다. 기존 가드(`export_scope_claim_test.dart`의
  `휴지통에 든 확정분은 내보내지 않는다`)가 그 절을 행동으로 고정하고 있다.
- **휴지통은 이벤트 한 줄로만 보여준다.** 거르지 않으면 하나를 지웠는데 `일정` 한 줄 +
  `이벤트` 한 줄이 따로 떠서 복구도 두 번 눌러야 한다. 순수 함수
  `visibleTrashSchedules`(`trash/domain/trash_filter.dart`)가 **삭제된 이벤트의 `schedule_id`
  집합에 드는 일정 행을 뺀다.**
  - **`TrashSnapshot`의 생성자가 스스로 부른다** — 호출부에서 거르게 두면 다음 호출부가
    빠뜨려 두 줄이 돌아온다. 배선을 문서가 아니라 구조가 진다.
  - ⚠️ **짝이 없는 일정은 그대로 보인다.** 입력 탭 검토 목록에서 ← 스와이프로 지운 검토
    대기 일정이 여기 속하고, 그것을 되돌릴 길은 휴지통 하나뿐이다(실행취소를 걷어냈다).
  - ⚠️ `permanentDeleteEvent`가 원본까지 지우는 이유가 이것이다 — 이벤트만 지우면 숨어
    있던 원본이 **고아가 되어 일정 섹션에 다시 나타난다.**
- **30일 자동 정리는 그대로 둔다**(사용자 결정). 짝이 같은 시각에 내려가므로 함께 사라진다.
  작년 자료가 다시 필요하면 에듀파인 CSV를 다시 가져오면 되고, 원본 CSV를 보관하는
  `imported_schedules`는 애초에 이 정리의 대상이 아니다.
- 가드: `test/features/calendar/data/event_delete_cascade_test.dart` **5**(삭제·재등록·
  손입력 무영향·복구·영구삭제) · `test/features/trash/trash_filter_test.dart` **5**
  (짝 숨김·짝 없음 유지·빈 목록·손입력·`TrashSnapshot` 배선).
  - ⚠️ 복구·영구삭제 테스트는 **전제 검사를 앞에 둔다.** 연쇄 삭제가 없으면 그 둘이
    **우연히 통과한다**(원본이 애초에 안 내려가니 "활성이다"·"휴지통에 없다"가 맞아버린다).
    실제로 처음 쓴 판본이 그랬다.
- 실측(iPhone 17 / iOS 26.5, 2026-09-11): 사진 AI 등록 → 확정 → 캘린더 삭제 → **재등록 성공**,
  휴지통 총 6건에 이벤트 한 줄만, 복구하니 휴지통 5건으로 줄고 캘린더에 되돌아왔다.
...
**단축어 등록은 검토 관문을 건너뛰고 바로 확정한다**(2026-09-13, 사용자 요청).
앱 밖에서 넣는데 확정하러 앱을 열어야 하면 "앱을 열지 않는다"는 전제가 무너지고,
**`일정 조회`가 `calendar_events`를 보므로 넣은 것을 조회할 수도 없었다** — 두 인텐트가
서로 다른 테이블을 본다는 것을 설계 때 확인하지 않아 생긴 부조화다.

- **사진 AI 히어로 경로는 그대로 검토 대기다.** 거기는 이미 앱 안이고 한 번에 스무 건씩
  들어와 관문이 제 몫을 한다. `intakeAiScheduleText`의 `status` 기본값이 `pending`인 이유.
- 되돌리는 길은 **캘린더 ← 스와이프**다. 연쇄 삭제가 원본까지 함께 내려보내므로 관문을
  빼도 값이 남는다(2026-09-11 절 참고).
- **결과 문구를 `AppIntentsStrings.registerSummary`로 분리했다.** `ImportStrings.
  aiRegisterSummary`는 `검토 목록에 넣었어요`라고 말하는데 단축어 도착지는 캘린더라
  그대로 쓰면 거짓이 된다. 가드가 **문구와 도착지를 양방향으로** 검사한다 —
  단축어 문구에 `검토`가 있으면 실패, 히어로 문구에 `검토`가 없으면 실패.
...
- ⚠️ **시리 문구에는 앱 이름이 반드시 들어간다**(애플 제약, 빠지면 컴파일 실패).
  설치 후 첫 호출에 뜨는 `이 단축어를 켤까요?`는 정상 확인 절차다.

#### 시리는 앱 이름보다 동작 명사를 더 무겁게 본다 — 낱말이 라우팅을 정한다 (2026-09-14)
...
|---|---|
| 공직플랜 일정 보기 | 우리 앱 |
| 공직플랜에 **일정** 등록 / 추가 | **애플 캘린더** — 앱 이름을 붙여도 |
| 공직플랜에 추가 / 넣어줘 (명사 없음) | **일기 앱** |

- 한국어 시리에서 `일정 등록`·`일정 추가`는 **캘린더 앱이 소유한 명령**이다. 시스템 앱의
  명령과 정면으로 겹치면 앱 이름은 지지 않는다. `일정 보기`는 캘린더가 그만큼 강하게
  잡지 않아 통과했다.
- 명사를 빼면 남는 동사(`추가`)를 가진 다른 시스템 앱이 가져간다. **낱말을 빼는 것은
  해결이 아니라 다른 충돌이다.**
- 그래서 음성 등록 문구는 이 앱의 **자기 용어 업무·행사**를 `\(\.$kind)` 자리로 쓴다
  (`공직플랜에 행사 추가`). 두 시스템 앱 어느 쪽도 이 낱말을 잡고 있지 않고, 시리가 들은
  낱말이 종류까지 채운다. 가드가 음성 추가 문구에 `일정`이 없는지 본다.
- ⚠️ **오진 기록.** "이름을 뭐라고 할까요?" → "언제로 설정할까요?" → 기기 캘린더 저장이라는
  흐름을 보고 **우리 인텐트가 도달했다**고 읽었다. 화면 상단의 `공직플랜에 일정 등록 ✎`을
  단축어 제목으로 봤는데 **시리가 받아적은 말**이었다(연필은 받아적기 수정). 우리 인텐트는
  파라미터가 하나라 두 번째 질문을 할 수 없다 — **질문 개수가 어느 앱의 흐름인지 가른다.**
  그 오진 위에 세운 "필수 파라미터에 `requestValueDialog`가 없어 시리가 포기한다" 가설도
  틀렸다(`requestValueDialog`는 남겨 뒀다 — 해롭지 않고 실제로 물을 때 문구가 된다).
- **음성용 인텐트는 따로 있다**(`AddScheduleIntent`: 제목 + `Date`). `RegisterSchedulesIntent`는
  AI가 만든 JSON을 받는 것이라 음성으로 부를 수 없다 — 문구를 `AI 목록 등록` 하나로 줄여
  실수로 말할 일을 없앴다. `Date` 파라미터는 시리가 "내일"·"다음 주 월요일"을 알아서 푼다.
- ⚠️ **자기 용어도 안 통했다**(161, iOS 26.6.2 실기기). `행사 추가` → 캘린더, `업무 추가` →
  미리 알림, 조회 `일정 보기`도 캘린더로 갔다(처음엔 우리 앱이었는데 **안정적이지 않다**).
  **iOS 26 한국어 시리에서는 App Shortcut 문구로 시스템 도메인을 이길 수 없다** — 앱 이름을
  붙여도, 어떤 낱말을 써도. 어휘 실험은 세 빌드로 종결했다(왕복 한 시간씩 전부 실패).
  App Shortcut 문구는 **단축어 앱 타일 노출용**으로만 기대한다.
- **시리는 보류다**(사용자 결정 2026-09-14). 근본 해법은 **캘린더 도메인 스키마**(iOS 27,
  Apple Intelligence 위에서 동작 — 엔티티·Spotlight 색인·쿼리까지 범위가 크다). 기기가 27로
  올라간 뒤 검토한다. 단축어 앱 직접 실행·공유 시트 경로는 전부 정상이라 기능은 산다.
- **어휘와 무관한 우회로**: 단축어 앱에서 사용자가 만든 단축어는 **자기가 붙인 이름**으로
  불리고 시스템 명령과 겹치지 않는다. 음성이 지금 필요하면 이 길이다(배포 없이 된다).
...
- ⚠️ **에듀파인은 출처 절에 명시한다**(규칙 뒤집힘, 2026-09-15). 예전엔 "링크하면 연동으로
  오해받는다"며 빼두었는데 Play가 **프로덕션 156을 정확히 반대 이유로 거부했다** —
  `제공된 출처가 불충분함`. 심사관의 기준은 접속 여부가 아니라 **앱이 표시하는 정보의
  출처**다. 이름만 나오고 URL이 없으면 빠진 출처로 읽힌다. 연동 오해는 URL 바로 아래
  "접속하지 않는다" 문장으로 막는다. 법령 링크도 루트가 아니라 **개별 규정 페이지**
  (`lsiSeq` ASCII 주소)로 — 한글 경로는 콘솔이 링크를 반만 건다.
  가드가 둘 다 지킨다(`store_listing_credit_test.dart`).
...
실측 warm 10.2초. 전체 1151건은 배포 리듬을 해쳐 뺐다 — **기능 회귀는 이 게이트가 잡지
