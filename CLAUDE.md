...
- **성격 카드는 한 줄이다** — 아이콘 + 종류 세그먼트 + ★ + 중요 스위치가 한 테두리 안
  ...
  - **`종류` 글자를 뺐다** — 세그먼트가 `업무`/`행사`라 스스로 설명하고, 320pt 폭에서
    글자까지 넣으면 넘친다. 글자가 사라진 중요 스위치는 `Semantics(label:)`로 이름을
    남긴다(목록의 ★을 감싼 것과 같은 이유).
...
- ⚠️ **앱 이름이 `공직플랜`이라 정부 앱으로 오인될 여지가 있다** — 면책조항을 지우면
  ...
  요건 때문이다.
- ⚠️ **iOS 설명(`docs/app_store_description.md`)에는 이 면책조항이 없다.** Apple이 아직
  문제 삼지 않았을 뿐 같은 콘텐츠다 — 다음 iOS 제출 때 함께 넣을 것.
- **기존 가드는 이 실패를 잡지 못한다.** `data_source_credit_test.dart`는 앱 안 문구만
  본다. 스토어 설명은 어떤 테스트도 검사하지 않으며, 이 검사는 **로컬에서 불가능한
  것이 아니다**(문서 파일을 읽으면 된다) — 재발하면 가드로 올릴 후보다.
