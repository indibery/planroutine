...
| 날짜 | intl | 한국어 로케일 |
| 테스트 | flutter_test, integration_test, sqflite_common_ffi | **1142** 유닛/위젯 + 19 E2E (실측 2026-09-13, 단축어 가드 45건을 더한 값. 직전 표기 `1097`은 2026-09-11 값이다. ⚠️ 이 숫자를 지키는 가드가 없어 **다섯 번** 낡았다 — README는 더 심해서 `1003`에 멈춰 있었다) |
...
- ⚠️ **시리 문구에는 앱 이름이 반드시 들어간다**(애플 제약, 빠지면 컴파일 실패).
  그래서 사용자가 `일정 보기`라고만 말하면 시리가 **애플 캘린더를 연다** — 앱에서
  고칠 수 없다(실기기 신고 2026-09-13). 대신 문구를 액션당 4개로 늘렸다(애플 권장 3~5).
...
실측 warm 10.2초. 전체 1142건은 배포 리듬을 해쳐 뺐다 — **기능 회귀는 이 게이트가 잡지
