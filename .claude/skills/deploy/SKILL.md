...
#### 기기에 새 빌드가 안 뜨면 — 원인은 트랙이 아니라 테스터 설정이다

실측(2026-09-04): `internal`로 올렸는데 실기기 Play 스토어가 **프로덕션 버전을 계속
보여줬다.** 레인·업로드는 정상이었다(`check_play_key`가 `internal status=completed`로
새 versionCode를 확인). 문제는 콘솔의 **테스터 목록**이었고, 설정을 마치자 곧바로 떴다.

⚠️ **이 상태는 Play API로 판별할 수 없다.** 트랙 조회의 테스터 필드에는
**Google Groups만** 나온다 — 콘솔에서 이메일 목록으로 넣은 테스터는
`google_groups=[]`로 보인다(실측: 실제 테스터가 있는 alpha도 빈 배열이었다).
그래서 "API가 비었으니 테스터가 없다"는 결론은 성립하지 않는다.

- **콘솔에서 테스터 수를 직접 볼 것** — `테스트 및 출시 › 내부 테스트 › 테스터`.
- 가장 흔한 누락은 이메일 목록을 **만들기만 하고 트랙에 체크하지 않는 것**이다.
- 기기 쪽: 테스터 계정으로 opt-in 링크를 한 번 열어야 하고, Play 스토어 앱은
  캐시가 있어 즉시 갱신되지 않는다.
...
- ⚠️ **그것과 별개로 서비스 계정에 프로덕션 출시 권한이 있어야 한다**(실측 2026-09-11).
  없으면 가드를 다 통과한 뒤 승격 호출에서
  `Google Api Error: Invalid request - The caller does not have permission`으로 끝난다.
  **읽기와 테스트 트랙 쓰기는 그대로 된다** — 트랙 조회도 되고 `beta`도 성공하므로
  레인 문제로 오진하기 쉽다. Play가 릴리스 권한을 **트랙별로** 가르기 때문이다.
  콘솔 `사용자 및 권한 › <서비스 계정> › 앱 권한`의 릴리스 항목에서 켠다.
  ⚠️ **API로는 자기 권한을 조회할 수 없다** — 확인은 콘솔에서만 된다.
...
#### 새 앱을 Play에 낼 때 거치는 관문 셋

세 가지가 **서로 다른 곳에 있고 순서대로 막는다.** 앱마다 다시 해야 하므로 다음 앱에서
여기부터 짚는다.

| 관문 | 어디서 | 증상 |
|---|---|---|
| 첫 업로드 수동(패키지 바인딩) | 콘솔에서 AAB 1건 업로드 | `insert_edit`에서 **404** |
| 서비스 계정 **프로덕션 출시 권한** | 콘솔 `사용자 및 권한 › 앱 권한` | 승격에서 **permission 거부** |
| **Android 개발자 인증** 패키지 이름 등록 | Play Console 홈 → `Android 개발자 인증` | 등록 없으면 **Play에서 앱 삭제** |

- 셋째는 2026-03 신설된 제도이고 **기한이 2026-09-30**이다. Play 앱 서명(= AAB 업로드)을
  쓰는 앱은 대부분 **자동 등록**되므로 보통은 콘솔 홈에서 상태만 확인하면 된다
  (공직플랜 `com.planroutine.app`은 **등록됨** 확인, 2026-09-11). 자동 등록이 안 된 앱은
  소유권 증명이 붙으므로 기한 직전에 확인하지 말 것.
- Play 밖으로도 배포하는 앱이라면 그 패키지와 **서명 키도 따로 등록**해야 한다 — 미등록
  조합은 일부 국가의 인증 기기에서 설치 자체가 막힌다. 공직플랜은 Play 전용이라 해당 없다.
