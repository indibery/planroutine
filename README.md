...
| **아이폰 단축어(App Intents)** | `일정 등록`·`일정 조회`를 단축어·시리에 노출. **둘 다 앱이 화면에 뜨지 않는다** — AI 앱 답변을 그대로 넘겨 검토 대기에 넣거나, 기간을 골라 일정 목록을 텍스트로 받는다. iOS 16.0+ |
...
- 테스트: flutter_test(유닛/위젯 **1129**) · integration_test(iPhone E2E 19) · sqflite_common_ffi
