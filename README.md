...
| **검토 후 확정** | 바로 등록하지 않고 입력 탭 검토 목록에서 슬라이드로 확정(→)/삭제(←). 하단 `일괄 업무 등록 N건`/`일괄 행사 등록 N건`으로 종류별 일괄 확정, 확정 시 캘린더 이벤트 자동 생성(종류 승계) |
...
- **Flutter 3.44.8** (Dart 3.12.2) — iOS 배포 중(App Store), Android는 Play 비공개 테스트 업로드. 두 스토어 모두 build/versionCode **143**
